...
import { addFoodEntry, deleteFoodEntry, parseFood } from '@/lib/db'
...
export default function FoodLogger({ date, entries, onAdd, onRemove }: FoodLoggerProps) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const entry = await addFoodEntry({ date, raw_input: input.trim(), meal_type: mealType, ...nutrition })
      if (entry) {
        onAdd(entry)
        setInput('')
      ...
      toast.error(msg.includes('API_KEY') ? 'Add NEXT_PUBLIC_CALORIE_NINJAS_API_KEY to .env.local' : msg)
    } finally {
      setParsing(false)
    }
  }
    ...
    <div className="space-y-3">
      ...
      {entries.length > 0 && (
        <div className="grid grid-cols-4 gap-2 pop-in">
          {[
            { icon: <Flame size={11} />, label: 'kcal', value: Math.round(totals.calories), color: '#f97316' },
            { icon: <Beef size={11} />, label: 'protein', value: `${Math.round(totals.protein)}g`, color: '#ef4444' },
            { icon: <Wheat size={11} />, label: 'carbs', value: `${Math.round(totals.carbs)}g`, color: '#eab308' },
            { icon: <Droplets size={11} />, label: 'fat', value: `${Math.round(totals.fat)}g`, color: '#06b6d4' },
          ].map(({ icon, label, value, color }) => (
            <div key={label} className="bg-card rounded-xl p-2.5 text-center border border-border">
            ...
            {items.map(entry => (
              <div key={entry.id}
                ...
                <div className="min-w-0">
                  <p className="text-[13px] font-medium truncate">{entry.food_name ?? entry.raw_input}</p>
      ...
      <div className="flex gap-2 items-center">
        <Select value={mealType} onValueChange={(v) => v && setMealType(v as FoodEntry['meal_type'])}>
          <SelectTrigger className="w-[110px] shrink-0 bg-card border-border text-[12px]">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
        ...
        <Input
          ...
          className="flex-1 bg-card border-border text-[13px]"
          disabled={parsing}
        />
        <Button
          onClick={handleAdd}
          ...
          {parsing ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />}
        </Button>
      </div>
    </div>
  )
