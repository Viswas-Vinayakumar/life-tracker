...
- [ ] **Idea:** auto-fetch real nutrition from the internet to pre-fill the correction fields (compare local estimate vs CalorieNinjas / web source, let user accept). Would make the 5-star correction flow one-click.
- [ ] Dedicated full "Learning" page (currently the ML view lives as a filter+summary inside the Activity Log).
...
| Date | Change |
|---|---|
| 2026-06-17 | **5-star accuracy rating + value correction + ML/Learning section.** Replaced the binary confirm with a per-entry **rating panel** (expands inline under the food row, on-theme): 1–5 stars + editable kcal/P/C/F/Fi fields so the user can correct partial values (e.g. checked online). Corrected values → become verified ground truth (`rateNutrition` in `lib/verifiedNutrition.ts`, now stores `rating`/`corrected`/`ratedAt`), **update the food entry in DB** (`updateFoodEntry` added to `lib/db.ts`) so totals fix instantly, and reload the AI summary. `parseFood` trusts a record when `corrected || rating>=4`. New activity type **`ml`** + actions `rated`/`corrected`/`learned` (`lib/activityLog.ts`); Activity Log gets a **"Learning" chip** (teal `#2dd4bf`, Brain icon) and, when active, a **Model Learning summary card** (foods learned / trusted / corrected / avg rating + list of learned foods). `noteVerifiedFood` now records the rating into AI memory. DMG built fine this time. **Idea parked:** auto-fetch reference values from the internet (CalorieNinjas/Gemini-style) to pre-fill corrections — not yet built. |
